...
namespace supra
{
	...
	template
	shared_ptr<Container<uint8_t> > ImageProcessingCuda::process<int16_t, uint8_t>(const shared_ptr<const Container<int16_t> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<uint8_t> > ImageProcessingCuda::process<float, uint8_t>(const shared_ptr<const Container<float> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<uint8_t> > ImageProcessingCuda::process<uint8_t, uint8_t>(const shared_ptr<const Container<uint8_t> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<float> > ImageProcessingCuda::process<int16_t, float>(const shared_ptr<const Container<int16_t> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<float> > ImageProcessingCuda::process<float, float>(const shared_ptr<const Container<float> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<float> > ImageProcessingCuda::process<uint8_t, float>(const shared_ptr<const Container<uint8_t> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<int16_t> > ImageProcessingCuda::process<int16_t, int16_t>(const shared_ptr<const Container<int16_t> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<int16_t> > ImageProcessingCuda::process<float, int16_t>(const shared_ptr<const Container<float> >& inImageData, vec3s size, WorkType factor);
	template
	shared_ptr<Container<int16_t> > ImageProcessingCuda::process<uint8_t, int16_t>(const shared_ptr<const Container<uint8_t> >& inImageData, vec3s size, WorkType factor);
}